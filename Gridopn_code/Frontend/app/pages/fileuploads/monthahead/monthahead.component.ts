...
export class MonthaheadComponent {
  ...
  constructor(
    ...
    private monthAheadForecastService: MonthAheadForecastService
  ) { }
  ...
  fetchAndSetDate() {
    this.monthAheadForecastService.fetchMonthAheadDate({}).subscribe({
      next: (res) => {
        if (res?.month_ahead) {
          const fromDate = new Date(Date.parse(res.month_ahead.from));
          const toDate = new Date(Date.parse(res.month_ahead.to));
      ...
      error: (err) => {
        console.error('Error fetching Month ahead date:', err);
      }
    });
  }
  ...
  handleFileInput(event: any) {
    ...
    // --- FIX: Ensure Dates are Selected BEFORE File Parse ---
    const dates = this.validationform.get('disabledDate')!.value;
    if (!dates || !dates.from || !dates.to) {
        this.toastService.show('Please select a valid Month range first.', { classname: 'bg-warning text-white', delay: 5000 });
    ...
    // --- FIX: Append Dates to FormData for Backend Validation ---
    ...
    this.monthAheadForecastService.parseMonthAheadExcel(formData).subscribe({
      next: (res: any) => {
        ...
        // --- Frontend Data Load & Confirmation ---
        // Note: Backend now handles date overwriting logic in 'parseMonthAheadExcel',
        // so res.data should already have correct dates.
    ...
    if (this.validationform.valid) {
      Swal.fire({
        ...
        if (result.value) {
          ...
          // --- FIX: Send JSON as a File Blob ---
          const jsonString = JSON.stringify(this.spreadsheet.nativeElement.jexcel.getData());
          const jsonBlob = new Blob([jsonString], { type: 'application/json' });
          
          // 'dataFile' matches the key we check in Python
          formData.append("dataFile", jsonBlob, "month_data.json");
          // -------------------------------------

          this.loadingText = 'Uploading...';
          this.uploading = true;

          this.monthAheadForecastService.uploadMonthAheadData(formData).subscribe((res: any) => {
            this.uploading = false;
        ...
        this.toastService.show('Please fill all required fields.', { classname: 'bg-warning text-white', delay: 5000 });
    }
  }
